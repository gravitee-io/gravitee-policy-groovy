// Vendored from org.kohsuke:groovy-sandbox 1.30 (MIT License, https://github.com/jenkinsci/groovy-sandbox).
// Local modification: backport of SECURITY-3792 (upstream commit 787843e) — interception of the
// per-element implicit cast emitted for a typed for-each loop, which otherwise lets a script invoke
// arbitrary constructors outside the sandbox. The call site is adapted to 1.30's visitForLoop, which
// still delegates to super.visitForLoop; the rewrite runs before that delegation so the injected cast
// is transformed with the loop body.
...
abstract class ScopeTrackingClassCodeExpressionTransformer extends ClassCodeExpressionTransformer {
     ...
     * cast) and prepend an explicit {@code v = (T) v} to the loop body, which the SandboxTransformer
     * visitor then rewrites to a {@code Checker.checkedCast} when it visits the body.
     */
